...
| 4 | Parser event genişletmesi | ✅ Tamam |
| 5 | Utility ekranı | ✅ Tamam |
| 6 | Aim ekranı | ✅ Tamam |
| 7 | Gelişmiş analiz + Ruby coaching | ✅ Tamam (7.1 metrikler + 7.2 Ruby koçluk notları) |
| 8 | Testler ve performans iyileştirmeleri | 🟡 Rust motoru eklendi (`analysis-rs`, varsayılan kapalı); tam geçiş sonraki aşama |
| 9 | `0.7.0-alpha.1` sürüm güncellemesi | ✅ Tamam (`docs/RELEASE-0.7.0-alpha.1.md`) |
| 10 | Windows portable ve NSIS GitHub build'i | ✅ Otomatik çalışıyor |

**Kalan: 0 zorunlu başlık.** Planın 10 maddesi tamamlandı; Aşama 8'in kalan kısmı
(round kazananı, entry/trade, clutch, KAST, ekonomi sınıflandırmanın Rust'a taşınması)
`coverage.deferred` altında izleniyor.
...
- [x] **Ekonomi**: round başı ekipman değeri, force/eco/full tespiti (`item_purchase`) → `buildEconomyModel` + Analysis "Ekonomi" bloğu
- [x] **Side split**: T ve CT performansı ayrı tablolar (takım + oyuncu) → `buildSideSplitModel`
- [x] **Round momentum grafiği** (`ui/components/chart.js`): round bazında skor farkı + kazanma serileri → `buildMomentumModel`
- [x] **Heatmap**: kill/ölüm noktaları maç genelinde → `buildMatchHeatmap`
- [x] **Opening duel** ayrımı: entry kill'lerin T/CT dağılımı → `buildOpeningDuels`

Uygulama notları (Oturum D):

- Modeller `ui/analysis/advanced-analysis.js` içinde; her biri `{ available, ... }` döner ve eksik veride Analysis ekranında `emptyState('warn')` gösterilir.
- `classifyBuy` eşikleri oyuncu başına harcama üzerinden: `eco < 1500$`, `full ≥ 4000$` (configurable `DEFAULTS`, ürün kuralı değil — tablo altında yazılı).
- Side split, devre arası taraf değişimini `round.roster` / `teamBySide` üzerinden hesaplar.
- `player_death` artık `user_X/Y/Z` + `attacker_X/Y/Z` istiyor (`electron/demo-worker.cjs`); ısı haritası ve kill mesafesi bu alanlardan besleniyor.
- Testler: `test/advanced-analysis.test.mjs` (11) + `test/analysis-advanced-view.test.mjs` (8).

### 7.2 Ruby coaching entegrasyonu — ✅ TAMAM

Mevcut altyapı hazırdı: Rust `ruby_analyze` action'ı payload'ı `backend/analytics/analyze.rb`'ye stdin'den geçiriyor.

```
JS: normalize metrikler → window.matchframe.core.request('ruby_analyze', { metrics, availability, scope })
Rust: run_ruby(payload) → analyze.rb
Ruby: kurallar → { engine, notes: [{ severity, category, tag, text, metric }], evaluated, skipped }
JS: notları Aim / Utility / Analysis ekranlarında göster
```

Yapılanlar:

1. [x] `ui/analysis/coaching.js`: modelden metrik özeti (`buildCoachingSummary`) + IPC
       çağrısı (`requestCoaching`) + oturum durumu (`ensure`/`refresh`/`subscribe`).
       Metrik yoksa gönderilmez; `availability` ve `missing` alanları Ruby'ye hangi
       kuralın atlanacağını söyler.
2. [x] `backend/analytics/analyze.rb`: 11 kural, 5 kategori (`aim`, `utility`, `entry`,
       `economy`, `positioning`), Türkçe mesajlar + tetikleyen metrik adı. Motor
       `evaluated` / `skipped` sayılarını da döndürür (kaç kuralın veri eksikliği
       nedeniyle atlandığı UI'da yazılır).
3. [x] UI: `ui/components/coach-notes.js` — kategoriye göre ilgili ekranda gösterim
       (Analysis: tümü · Aim: aim · Utility: utility), "Yenile" butonu.
4. [x] **Ruby yoksa ekran çalışmaya devam eder** (`run_ruby` hata döndürür → JS
       `unavailable` durumuna düşer + durum notu gösterir).

### 7.3 Testler — ✅ TAMAM

- [x] Ruby kuralları için fixture girdi/çıktı testi: `test/analyze-rb.test.mjs`
      (`ruby` varsa çalışır, yoksa atlanır; CI'da Ruby 3.4 kurulu).
- [x] JS tarafı: `test/coaching.test.mjs` (metrik özeti, availability, normalizeNotes,
      IPC hata yolları) ve `test/coaching-view.test.mjs` (üç durum: yükleniyor /
      motor yok / notlar hazır).

---

## Aşama 8 — Rust'a taşıma (performans) — 🟡 KISMİ (aşamalı)

**Ön koşul:** formüller JS tarafında testlerle doğrulanmış olmalı (Aşama 5-7 sonrası) → ✅.

1. [x] `backend/src/analysis.rs`: `build_match_model` — serde ile normalize edilmiş event girdisi → aynı JSON modeli
       (round/oyuncu/takım kill, ölüm, hasar, ADR, headshot)
2. [x] Rust action: `"analysis_build"` → versioned analysis schema (`schemaVersion`)
3. [x] Electron: `ui/analysis/rust-bridge.js` + `ui/state/demo-store.js` içinde **gölge
       karşılaştırma** (varsayılan kapalı, `localStorage.mf.rustModel`). JS modeli kaynak
       doğru olmaya devam eder.
4. [x] Ruby'ye yalnızca normalize metrikler verilir (ham event değil)
5. [x] Karşılaştırma testi: `test/rust-parity.test.mjs` (`MF_CORE_BIN` ile çalışır)

Kalan (`coverage.deferred`): round kazananı çıkarımı, entry/trade, clutch, KAST ve ekonomi
sınıflandırmanın Rust'a taşınması. Modül `analysis-rs` cargo özelliğiyle derlenir;
kapalıyken release hattı değişmez.

---

## Oturum planlaması

> **Durum (2026-09-05):** Oturum A-F tamamlandı. Aşama 4, 5, 6, 7.1, 7.2, 8 (kısmi) ve 9
> `arena/01a06f0a-matchframe` dalında; `npm test` 147 test · 144 geçer · 3 atlanır.
> **Kalan oturum yok.** Aşama 8'in kalanı (`coverage.deferred`) yeni bir plan ister.
> Sandbox çıkışı GitHub'a kapalı (TLS), bu yüzden dal kullanıcı makinesinden itiliyor:
> her oturum sonunda zip üretilip indirme panelinden veriliyor, kullanıcı `git push`
> ediyor. Windows build'i dal GitHub'a itildiğinde workflow tarafından otomatik alınıyor.
...
| **D** | Aşama 7.1: ekonomi, side split, momentum grafiği, maç heatmap | Analysis ikinci sürüm tamam | ✅ (bu dalda, 124 test) |
| **E** | Aşama 7.2 + 8: Ruby coaching + Rust'a taşıma + regresyon/perf | Koçluk notları görünüyor, büyük demoda playback düşüşü yok | ✅ (bu dalda, 147 test) |
| **F** | Sürüm finalizasyonu: `0.7.0-alpha.1` + tüm testler + build/release | Portable + NSIS EXE yayınlandı | ✅ (`docs/RELEASE-0.7.0-alpha.1.md`) |
