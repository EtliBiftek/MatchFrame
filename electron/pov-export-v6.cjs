...
async function ensureVrf() {
  ...
  if (digest.toLowerCase() !== VRF_SHA256.toLowerCase()) throw new Error('Source 2 Viewer indirmesi SHA-256 doğrulamasından geçmedi.');
...
function sanitizeGeometryJson(gltf) {
  ...
  for (const mesh of gltf.meshes || []) {
    ...
    for (const primitive of mesh.primitives || []) {
      primitive.attributes && delete primitive.attributes.COLOR_0;
      primitive.attributes && delete primitive.attributes.TANGENT;
      primitive.attributes && delete primitive.attributes.TEXCOORD_0;
      primitive.attributes && delete primitive.attributes.TEXCOORD_1;
...
async function exportMapGlb(cli, mapVpk, internal, gameInfo, outputDir) {
  ...
  let firstError = null;
  try {
    await execFileAsync(cli, args, { timeout: 20 * 60 * 1000 });
  } catch (error) {
    firstError = error;
  }
  const glb = await findRecursive(outputDir, (_full, name) => name.toLowerCase().endsWith('.glb'));
  if (glb) return glb;

  // Fallback: preserve the complete VPK on disk first. A .vmap_c needs its maps/<map>/
  // dependency directory; exporting only the compiled .vmap_c leaves the exporter without
  // world-node/entity/mesh dependencies.
  const rawDir = path.join(path.dirname(outputDir), `source-${CACHE_VERSION}`);
  await fs.promises.rm(rawDir, { recursive: true, force: true });
  await fs.promises.mkdir(rawDir, { recursive: true });
  let extractionError = null;
  try {
    await execFileAsync(cli, ['-i', mapVpk, '-o', rawDir], { timeout: 10 * 60 * 1000 });
  } catch (error) {
    extractionError = error;
  }
  const compiled = await findRecursive(rawDir, (_full, name) => name.toLowerCase() === `${path.basename(mapVpk, '.vpk')}.vmap_c`);
  if (!compiled) {
    const detail = extractionError || firstError;
    throw new Error(`VPK map exportu GLB üretemedi.${detail ? `\n${String(detail.message || detail).trim()}` : ''}`);
  }
  await fs.promises.rm(outputDir, { recursive: true, force: true });
  await fs.promises.mkdir(outputDir, { recursive: true });
  try {
    await execFileAsync(cli, ['-i', compiled, '-o', outputDir, '-d', '--gltf_export_format', 'glb', '--threads', '1', '--game', gameInfo], { timeout: 20 * 60 * 1000, cwd: rawDir });
  } catch (error) {
    const glbAfterFailure = await findRecursive(outputDir, (_full, name) => name.toLowerCase().endsWith('.glb'));
    if (!glbAfterFailure) throw new Error(`Standalone vmap_c glTF exportu başarısız.\n${String(error.message || error).trim()}`);
  }
  const fallbackGlb = await findRecursive(outputDir, (_full, name) => name.toLowerCase().endsWith('.glb'));
  if (!fallbackGlb) throw new Error(`Standalone vmap_c GLB çıktısı bulunamadı.`);
  return fallbackGlb;
}
...
async function prepare(mapName) {
  ...
  const job = (async () => {
    ...
    if (!(await isSafeGlb(cachedGlb))) {
      ...
      const rawGlb = await exportMapGlb(cli, mapVpk, `maps/${map}.vmap_c`, gameInfo, outputDir);
